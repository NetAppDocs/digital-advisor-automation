...
In GraphQL, mutations work like POST, PATCH, and DELETE requests in REST APIs. The mutation name determines which modification is run. To create a mutation, you must specify:
...
* The payload object. The data you want to return from the server, composed of return fields. You pass this as the body of the mutation name.
...
This process makes the query argument dynamic. You can simply change the value in the variables object and keep the rest of the query the same. Variables let you update values without changing the query.
