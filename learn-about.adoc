...
[.lead]
Digital Advisor leverages programmatic and artificial intelligence to analyze telemetry and product data. This provides proactive care and optimization for your data infrastructure and delivers actionable predictive analytics for NetApp environments through API access to telemetry-driven insights and install-base details. Digital Advisor APIs enable integration of these analytics into organizational workflows, dashboards, and fleet management systems.
...
The transition will be phased, with a period of overlap between REST APIs and GraphQL APIs. For each deprecated REST API, a deprecation notice will be published in the Digital Advisor API catalog, you need to migrate that specific endpoint to GraphQL within six months. To identify the REST APIs scheduled for deprecation and learn how to migrate them, see link:identify-deprecated-apis.html[Migrate deprecated Digital Advisor REST API endpoints to GraphQL].
...
The link:https://graphql.org[GraphQL^] data query language has the following attributes:
