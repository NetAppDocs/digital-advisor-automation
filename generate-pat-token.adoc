...
. Complete the authorization request form, and then select *Submit*.
+
NOTE: Your request should be approved instantly and activation is automatic. After you are authorized to use the Digital Advisor APIs, you can generate tokens to use when making programmatic API calls.
...
. Enter the number of days until expiration. The value must be a whole number from 1 to 90.
. Select *Generate Token*.
. Copy the generated token and save it in a secure location. Confirm that you saved it, and then select *Done*.
